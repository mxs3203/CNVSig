...
rule csv_to_pickle_filter:
    input: "data/input/ascat_tcga.csv"
    output: "data/output/csv_to_pickle_filter/tcga_ascat.obj"
...
rule compute_bin_sizes:
    ...
    run:
        ...
        for chr in tqdm(range(1,23)):  # for chr 1 to 22
            ...
            total.append([chr, cent_end, cent_start, chrom_size, cent_start - 1, chrom_size - cent_end,
                          (chrom_size - cent_end) - (cent_start - 1)])

        total = pd.DataFrame(total,columns=['chr', 'cent_start', 'cent_end', 'chrom_size', 'short_arm_l', 'long_arm_l',
                                            'diff_long_short'])
        ...
        if len(files_in_sample) == 22:
            ...
            for f in features:
                ...
                for specific_chr in order_of_chromosomes:  # for every chr with specific order
                    ...
                    chr_specific_bins = pd.read_csv("{}/{}.csv".format(
                        input[1],specific_chr))  # read bin file for that chr data/output/chrom_bin_def/4.csv
                    specific_feature_bin_row = makeSquareImage(chr_data,chr_specific_bins,f,
                        minMax_of_feature[f])  # returns 22 values representing all bins for specific features and chr
