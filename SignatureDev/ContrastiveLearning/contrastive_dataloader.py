...
class CNVImagesContrastive(Dataset):
    def __init__(self, img_dir, num_quantiles_for_wgii=3):
        ...
        meta_data = pd.read_csv("{}/data/input/hmf_metadata.csv".format(feature_util.linux_path), encoding="utf-16")
        self.annotation = self.annotation.merge(meta_data[["ID","TCGA_type"]], on='ID')
        self.annotation['quantile'] = pd.qcut(self.annotation['wGII'], num_quantiles_for_wgii, labels=False)
        self.annotation['TCGA_type'] = self.annotation['TCGA_type'].astype('category')
        self.annotation['TCGA_type_code'] = self.annotation['TCGA_type'].cat.codes
    ...
    def __getitem__(self, idx):
        ...
        return image1, image2, self.annotation.iloc[idx, 1], np.array(self.annotation.iloc[idx, 7], dtype="long"), np.array(self.annotation.iloc[idx, 8], dtype="long")